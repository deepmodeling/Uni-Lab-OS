...
```cmd
# 先独立同步模板
unilab --ak <ak> --sk <sk> --addr <api_url> template-sync

# 真机
python unilabos/app/main.py -g unilabos/devices/workstation/AI4M/AI4M.json `
  --ak <ak> --sk <sk> --addr <api_url> --disable_browser
...
`template-sync` 会把 `AI4M_station.yaml` 的 schema 上传到云端，之后实验记录本
就能看到所有 `auto-*` 动作。
...
- [ ] 8. 先运行 `unilab ... template-sync`，再用 `python unilabos/app/main.py -g <Graph> ...` 上线，到实验记录本下发动作回归。
