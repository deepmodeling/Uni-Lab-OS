...
-- 物料持有者/位置事实：与 resource_relation（父子层级）分开，支持设备工位、
-- 机械臂等非 material_instance 资源作为当前位置。
CREATE TABLE IF NOT EXISTS material_custody (
    material_uuid TEXT PRIMARY KEY,
    location_kind TEXT NOT NULL DEFAULT 'warehouse',
    location_uuid TEXT NOT NULL DEFAULT '',
    holder_kind TEXT NOT NULL DEFAULT '',
    holder_uuid TEXT NOT NULL DEFAULT '',
    version INTEGER NOT NULL DEFAULT 1
);
CREATE INDEX IF NOT EXISTS idx_custody_location ON material_custody(location_kind, location_uuid);
"""
...
CREATE INDEX IF NOT EXISTS idx_custody_location ON material_custody(location_kind, location_uuid);
-- Backfill custody facts for instances created before v6.  The v5 backend
-- compatibility layer exposes material_instance as a view, so this remains
-- valid for both the native and compatibility layouts.
INSERT OR IGNORE INTO material_custody(material_uuid, location_kind, location_uuid, version)
SELECT edge_uuid,
       CASE WHEN COALESCE(parent_uuid, '') <> '' THEN 'stack' ELSE 'warehouse' END,
       COALESCE(parent_uuid, ''),
       1
FROM material_instance;
"""
...
class InventoryStore:
    ...
    def _migrate(self) -> None:
        with self._lock:
            ...
            if (
                ...
                self._conn.executescript(_SCHEMA_V6_CUSTODY)
    ...
    def get_custody(self, material_uuid: str) -> Optional[Dict[str, Any]]:
        return self.query_one(
            "SELECT * FROM material_custody WHERE material_uuid = ?", (material_uuid,)
        )

    def list_custody(self, limit: int = 500) -> List[Dict[str, Any]]:
        return self.query_all(
            "SELECT * FROM material_custody ORDER BY material_uuid LIMIT ?", (limit,)
        )
