...
class WorkflowNodeWrite(BaseModel):
    ...
    # Action 成功后由 Edge Scheduler 登记的输出物料声明。
    material_outputs: JsonArray = Field(default_factory=list)
    material_preconditions: JsonArray = Field(default_factory=list)
    material_effects: JsonArray = Field(default_factory=list)
    ...
    @field_validator("material_outputs", "material_preconditions", "material_effects", mode="before")
    @classmethod
    def _material_outputs_array(cls, value: Any) -> JsonArray:
