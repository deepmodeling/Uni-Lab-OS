...
WorkflowService 形成第二个任务权威；后端 ``job_start/cancel/error_decision``
则统一由 ``ws_client`` 转交执行微后端。
...
from unilabos.server.scheduler.models import to_backend_workflow_status
from unilabos.server.scheduler.ordering import HttpSchedulerOrderer, StableLocalOrderer
...
def bind_workflow_executor(workflow_service: Any = None) -> Optional[Any]:
    ...
    if workflow_service is None:
        from unilabos.server.workflow.composition import get_workflow_service
...
def setup_edge_inventory(
    ...
    *,
    ...
    if sync_sender is not None and _outbox_worker is None:
        from unilabos.server.scheduler.inventory.sync import OutboxWorker
...
def setup_edge_scheduler(
    ...
    if not profile.can_recover_local_workflow_task:
        raise RuntimeError(
            ...
            f"{profile.value}; backend commands belong to the ws microbackend"
        )
    ...
    from unilabos.server.scheduler.estimation import DurationEstimator
...
def shutdown_edge_services() -> None:
    ...
    from unilabos.server.workflow.composition import reset_workflow_service_for_test
