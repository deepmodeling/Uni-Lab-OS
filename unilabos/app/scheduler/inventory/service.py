...
输出物料登记（实例、内容、父子/堆栈关系）和内容耗尽销毁也遵守同一事务边界。
...
class InventoryService:
    ...
    def site_occupant(self, owner_uuid: str, site_name_or_uuid: str) -> str | None:
        """返回指定容器具名 Site 的当前占用物料，空位返回 ``None``。

        该只读查询供调度准入使用：机器人动作在派发前检查目标位，避免把一个
        会在设备内部等待空位的动作送入单机器人执行器，从而阻塞负责清空该位
        的后续动作。
        """

        row = self.store.query_one(
            "SELECT occupied_material_uuid FROM site "
            "WHERE material_uuid=? AND (uuid=? OR name=?) AND deleted_at IS NULL",
            (owner_uuid, site_name_or_uuid, site_name_or_uuid),
        )
        if row is None:
            return None
        value = row.get("occupied_material_uuid")
    ...
    @staticmethod
    def _tx_get_custody(conn: sqlite3.Connection, material_uuid: str) -> Dict[str, Any]:
        row = conn.execute(
            "SELECT * FROM material_custody WHERE material_uuid = ?", (material_uuid,)
        ).fetchone()
        if row is None:
            raise NotFound(f"custody for instance {material_uuid} not found")
        return dict(row)
        ...
        with self._tx() as conn:
            ...
            conn.execute(
                ...
                (edge_uuid, legacy_cloud_id, lot_id, template_id, barcode,
                 InstanceState.WAREHOUSE.value, parent_uuid),
            )
            if parent_uuid:
                self._tx_upsert_relation(conn, parent_uuid, slot_id, edge_uuid)
            conn.execute(
                "INSERT OR IGNORE INTO material_custody(material_uuid, location_kind, location_uuid, "
                "holder_kind, holder_uuid, version) VALUES (?,?,?,?,?,1)",
                (edge_uuid, "stack" if parent_uuid else "warehouse", parent_uuid if parent_uuid else "",
                 "", ""),
            )
            self._emit(
    ...
    @_traced_operation("output.register")
    def register_material_output(
        self,
        template_id: str,
        ...
        *,
        ...
        """原子登记 Action 产出的实体物料及其内容、位置和来源。

        ``edge_uuid`` 是幂等身份；重复回放不会重复创建实例。新输出默认进入
        ``warehouse``，父物料/slot 由 ``resource_relation`` 记录。内容状态和
        实例登记、ledger/outbox 在同一 SQLite 事务提交，避免出现“实例已登记但
        内容或堆栈位置丢失”的半完成输出。
        """
        template_id = str(template_id or "").strip()
        if not template_id:
            raise InvariantViolation("material output requires template_id")
        edge_uuid = str(edge_uuid or f"material-output-{uuid.uuid4().hex}").strip()
        now = self._now_ms()
        encoded_content = dict(content or {})
        with self._tx() as conn:
            existing = conn.execute(
                "SELECT * FROM material_instance WHERE edge_uuid = ?", (edge_uuid,)
            ).fetchone()
            if existing is not None:
                ...
                if inst.get("template_id") != template_id:
                    raise CommandRejected(
                        f"output {edge_uuid} already belongs to template {inst.get('template_id')}"
                    )
                if parent_uuid:
                    self._tx_upsert_relation(conn, parent_uuid, slot_id, edge_uuid)
                    if inst["status"] != InstanceState.STORED.value:
                        inst = self._tx_set_instance_status(conn, inst, InstanceState.STORED)
                if parent_uuid:
                    conn.execute(
                        "INSERT INTO material_custody(material_uuid, location_kind, location_uuid, version) "
                        "VALUES (?,?,?,1) ON CONFLICT(material_uuid) DO UPDATE SET "
                        "location_kind=excluded.location_kind, location_uuid=excluded.location_uuid, "
                        "holder_kind='', holder_uuid='', version=material_custody.version+1",
                        (edge_uuid, "stack", parent_uuid),
                    )
                if encoded_content:
                    row = conn.execute(
                    ...
                    payload = json.dumps(encoded_content, ensure_ascii=False)
                    if row is None:
                        conn.execute(
                            "INSERT INTO substance_content(instance_uuid, state_json, version) VALUES (?,?,?)",
                            (edge_uuid, payload, version),
                        )
                    else:
                        conn.execute(
                            "UPDATE substance_content SET state_json=?, version=? WHERE instance_uuid=?",
                            (payload, version, edge_uuid),
                        )
                    self._emit(
                        conn, now, "content", edge_uuid, version, "content.updated",
                        {"state": encoded_content, "output_replay": True},
                        causation_id=causation_id, actor=actor,
                    )
                return {"status": "already_registered", "instance": inst}

            if barcode:
                placeholders = ",".join("?" for _ in _ACTIVE_STATES_TUPLE)
                duplicate = conn.execute(
                    f"SELECT edge_uuid FROM material_instance WHERE barcode = ? AND status IN ({placeholders})",
                    (barcode, *_ACTIVE_STATES_TUPLE),
                ).fetchone()
                if duplicate is not None:
                    raise DuplicateBarcode(
                        f"barcode {barcode} already active on {duplicate['edge_uuid']}"
                    )
            initial_status = (
                InstanceState.STORED.value if parent_uuid else InstanceState.WAREHOUSE.value
            )
            conn.execute(
                "INSERT INTO material_instance(edge_uuid, legacy_cloud_id, lot_id, template_id, "
                "barcode, status, parent_uuid, version) VALUES (?,?,?,?,?,?,?,1)",
                (edge_uuid, "", lot_id, template_id, barcode,
                 initial_status, parent_uuid),
            )
            if parent_uuid:
                self._tx_upsert_relation(conn, parent_uuid, slot_id, edge_uuid)
            conn.execute(
            ...
            self._emit(
                conn, now, "instance", edge_uuid, 1, "material.output.registered",
                {
            ...
            if encoded_content:
                conn.execute(
                    "INSERT INTO substance_content(instance_uuid, state_json, version) VALUES (?,?,1)",
                    (edge_uuid, json.dumps(encoded_content, ensure_ascii=False)),
                )
                self._emit(
                    conn, now, "content", edge_uuid, 1, "content.created",
                    {"state": encoded_content, "task_id": task_id, "run_id": run_id,
                     "action_id": action_id, "output_name": output_name},
                    causation_id=causation_id, actor=actor,
                )
            return {
                "status": "registered",
                "instance": self._tx_get_instance(conn, edge_uuid),
                "content": encoded_content,
            }

    @_traced_operation("content.consume")
    def consume_material_content(
        self,
        instance_uuid: str,
        quantity: float,
        *,
        ...
        """扣减输出物料内容；耗尽时自动将实体转为 ``consumed``。

        内容 JSON 必须在 ``quantity_key`` 下提供非负数。容器本身可通过
        ``auto_terminal=False`` 保留复用；默认行为适合一次性试剂实体。
        """
        if quantity <= 0:
            raise InvariantViolation("consumed quantity must be > 0")
        now = self._now_ms()
        with self._tx() as conn:
            ...
            if row is None:
                raise NotFound(f"content for instance {instance_uuid} not found")
            state = json.loads(row["state_json"])
            current = state.get(quantity_key)
            if isinstance(current, bool) or not isinstance(current, (int, float)):
                raise CommandRejected(f"content quantity key {quantity_key} is not numeric")
            remaining = max(0.0, float(current) - float(quantity))
            state[quantity_key] = remaining
            version = row["version"] + 1
            conn.execute(
                "UPDATE substance_content SET state_json=?, version=? WHERE instance_uuid=?",
                (json.dumps(state, ensure_ascii=False), version, instance_uuid),
            )
            self._emit(
                conn, now, "content", instance_uuid, version, "content.consumed",
                {"quantity": quantity, "unit": unit, "quantity_key": quantity_key,
                 "remaining": remaining},
                causation_id=causation_id, actor=actor,
            )
            if auto_terminal and remaining <= 1e-9:
                inst = self._tx_get_instance(conn, instance_uuid)
                inst = self._tx_set_instance_status(conn, inst, InstanceState.CONSUMED)
                conn.execute("DELETE FROM resource_relation WHERE child_uuid = ?", (instance_uuid,))
                conn.execute(
                    "UPDATE material_instance SET parent_uuid = '' WHERE edge_uuid = ?",
                    (instance_uuid,),
                )
                self._emit(
                    conn, now, "instance", instance_uuid, inst["version"],
                    "instance.consumed", {"reason": "content_exhausted"},
                    causation_id=causation_id, actor=actor, reason="content_exhausted",
                )
            return {
                "status": "consumed" if remaining <= 1e-9 else "updated",
    ...
    def material_custody(self, material_uuid: str) -> Dict[str, Any]:
        """只读返回物料当前位置/持有者事实。"""
        return self.store.get_custody(material_uuid) or {
    ...
    @_traced_operation("custody.set")
    def set_material_custody(
        self,
        material_uuid: str,
        *,
        location_kind: str,
        ...
        expected_location_uuid: str = "",
        expected_holder_kind: str = "",
        expected_holder_uuid: str = "",
        actor: str = "scheduler",
        causation_id: str = "",
    ) -> Dict[str, Any]:
        """原子更新物料位置/持有者，并可校验交接前置条件。

        ``expected_*`` 非空时必须与当前 custody 完全匹配，否则抛出
        ``CommandRejected``，从而阻止工位尚未完成时机械臂提前取料或覆盖
        另一个持有者的更新。
        """
        location_kind = str(location_kind or "").strip()
        if not location_kind:
            raise InvariantViolation("location_kind is required")
        now = self._now_ms()
        with self._tx() as conn:
            self._tx_get_instance(conn, material_uuid)
            row = conn.execute(
                "SELECT * FROM material_custody WHERE material_uuid = ?", (material_uuid,)
            ).fetchone()
            ...
            if expected_location_kind and current["location_kind"] != expected_location_kind:
                raise CommandRejected(
                    f"material {material_uuid} is at {current['location_kind']}, "
                    f"expected {expected_location_kind}"
                )
            if expected_location_uuid and current["location_uuid"] != expected_location_uuid:
                raise CommandRejected(
                    f"material {material_uuid} is at {current['location_uuid']}, "
                    f"expected {expected_location_uuid}"
                )
            if expected_holder_kind and current["holder_kind"] != expected_holder_kind:
                raise CommandRejected(
                    f"material {material_uuid} is held by {current['holder_kind']}, "
                    f"expected {expected_holder_kind}"
                )
            if expected_holder_uuid and current["holder_uuid"] != expected_holder_uuid:
                raise CommandRejected(
                    f"material {material_uuid} is held by {current['holder_uuid']}, "
                    f"expected {expected_holder_uuid}"
                )
            version = int(current["version"]) + 1
            conn.execute(
                "INSERT INTO material_custody(material_uuid, location_kind, location_uuid, "
                "holder_kind, holder_uuid, version) VALUES (?,?,?,?,?,?) "
                "ON CONFLICT(material_uuid) DO UPDATE SET location_kind=excluded.location_kind, "
                "location_uuid=excluded.location_uuid, holder_kind=excluded.holder_kind, "
                "holder_uuid=excluded.holder_uuid, version=excluded.version",
                (material_uuid, location_kind, str(location_uuid or ""),
                 str(holder_kind or ""), str(holder_uuid or ""), version),
            )
            self._emit(
                conn, now, "custody", material_uuid, version, "material.custody.changed",
                {
                    "from_location_kind": current["location_kind"],
                    "from_location_uuid": current["location_uuid"],
                    "from_holder_kind": current["holder_kind"],
                    "from_holder_uuid": current["holder_uuid"],
                    "location_kind": location_kind,
                    "location_uuid": str(location_uuid or ""),
                    "holder_kind": str(holder_kind or ""),
                    "holder_uuid": str(holder_uuid or ""),
                },
                causation_id=causation_id, actor=actor,
            )
            return {
    ...
    def select_instance_for_requirement(
        self,
        requirement: MaterialRequirement,
    ) -> Dict[str, Any]:
        """只读选择一个可共享的物料实例，不改变实例状态。

        ``reserve_workflow`` 的实例需求会把候选从 ``warehouse`` 推进到
        ``reserved``，适用于烧杯等任务独占物料。共享试剂只需要在任务准入时
        确定一个稳定 UUID，不能因为第一个任务而把同一瓶试剂锁死；该入口复用
        完全相同的库位/挂载点选择规则，但不产生状态迁移。调用方仍应在同一
        事务边界内完成后续共享数量预留或绑定。
        """

        if not isinstance(requirement, MaterialRequirement):
            raise CommandRejected("requirement must be MaterialRequirement")
        with self._tx() as conn:
            return self._tx_resolve_instance(conn, requirement)
    ...
    def set_instance_parent(
        ...
        with self._tx() as conn:
            ...
            # Keep the custody fact aligned with the single parent relation.  The
            # instance lifecycle status remains unchanged here: assigning a
            # parent is metadata, while deploy/consume owns warehouse/bench
            # transitions.
            conn.execute(
                "INSERT INTO material_custody(material_uuid, location_kind, location_uuid, "
                "holder_kind, holder_uuid, version) VALUES (?,?,?,?,?,1) "
                "ON CONFLICT(material_uuid) DO UPDATE SET location_kind=excluded.location_kind, "
                "location_uuid=excluded.location_uuid, holder_kind='', holder_uuid='', "
                "version=material_custody.version+1",
                (edge_uuid, "stack" if parent_uuid else "warehouse", parent_uuid or "", "", ""),
            )
            self._emit(
