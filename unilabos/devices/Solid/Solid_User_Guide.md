...
提供以固体配方为核心的完整流程：
  ...
  启动烧结：发送烧结启动命令。
接入形式：
  前端以 UniLabJsonCommand 的 auto-* 动作进行交互（定义见 [solid_params.yaml](Uni-Lab-OS/unilabos/registry/devices/solid_params.yaml)）。
  后端设备实现位于 [update_params.py](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)。

## 设备配置
设备图配置文件：[device.json](Uni-Lab-OS/unilabos/devices/Solid/device.json)
  ip：目标 TCP IP（默认 127.0.0.1）
  port：目标 TCP 端口（默认 8091）
注册文件：[solid_params.yaml](Uni-Lab-OS/unilabos/registry/devices/solid_params.yaml)
  module：unilabos.devices.Solid.update_params:SolidParamsDevice
  type：python
  action_value_mappings：定义 auto-update_params、auto-start_recipt、auto-start_sintering、auto-get_furnace_status 的参数与说明
## 初始化
默认使用设备图中的 ip/port 与设备进行 TCP 通信。
...
参考实现：[update_params.py](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
...
## 重要说明
长连接说明：
  所有接口均使用长连接复用同一 TCP 套接字（update_params、start_recipt、start_sintering、start_milling），异常时自动重连（实现见 [_send_recv/_ensure_socket](Uni-Lab-OS/unilabos/devices/Solid/update_params.py#L93-L113)）。
  发送按行结束符 CRLF，接收按 b"\\n"/b"\\r\\n" 聚合。
写文件策略：
...
相关实现：
  [update_params（参数下发/写文件）](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
  [start_recipt（启动、轮询、拉取结果）](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
  [start_sintering（启动烧结）](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
  [start_milling（启动球磨）](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
  [get_furnace_status（状态查询）](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
  [get_ball_bead（获取球珠数量）](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
  [set_ball_bead（设置球珠数量）](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
  [unload_sintering（下坩埚）](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
  [loading_material（上料）](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
  [unloading_material（下料）](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
  [start_material（启动上下料任务）](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)

## 参考路径
设备实现代码：[update_params.py](Uni-Lab-OS/unilabos/devices/Solid/update_params.py)
设备注册配置：[solid_params.yaml](Uni-Lab-OS/unilabos/registry/devices/solid_params.yaml)
设备图配置：[device.json](Uni-Lab-OS/unilabos/devices/Solid/device.json)
