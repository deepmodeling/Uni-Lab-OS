...
def _spec_to_dict(spec: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        "run_id": getattr(spec, "run_id", "") or spec.workflow_id,
        ...
        "nodes": [
            {
                "id": n.id,
                "run_id": getattr(n, "run_id", "") or getattr(spec, "run_id", "") or spec.workflow_id,
                ...
                "material_outputs": [dict(output) for output in getattr(n, "material_outputs", [])],
                "material_preconditions": [
                    dict(item) for item in getattr(n, "material_preconditions", [])
                ],
                "material_effects": [
                    dict(item) for item in getattr(n, "material_effects", [])
                ],
            }
            for n in spec.nodes
        ],
...
class WorkflowHistoryStore:
    ...
    def __init__(self, db_path: str = ":memory:", max_runs: int = 2000):
        ...
        with self._lock:
            ...
            try:
                ...
                pass  # 列已存在
            for table, column in (("workflow_runs", "run_id"), ("job_runs", "run_id")):
                try:
                    self._conn.execute(
    ...
    def record_submitted(self, spec: Any, state: str) -> None:
        ...
        with self._lock:
            ...
            self._conn.execute(
                ...
                "(workflow_id, run_id, task_id, lab_id, priority, node_count, state, submitted_at, "
                "started_at, finished_at, duration_s, spec_json) VALUES (?,?,?,?,?,?,?,?,?,NULL,NULL,?)",
                (
                    spec.workflow_id,
                    getattr(spec, "run_id", "") or spec.workflow_id,
                    spec.task_id,
                    spec.lab_id,
                    str(spec.priority),
                    len(spec.nodes),
    ...
    def record_job(self, entry: Dict[str, Any], ret_value: Any = None) -> None:
        ...
        with self._lock:
            self._conn.execute(
                "INSERT INTO job_runs(job_id, workflow_id, run_id, node_id, device_id, action_name, "
                "device_action_key, started_at, ended_at, actual_s, estimated_s, "
                "estimate_source, state, suc_type, ret_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    entry["job_id"],
                    entry["workflow_id"],
                    entry.get("run_id", entry["workflow_id"]),
                    entry["node_id"],
                    entry.get("device_id", ""),
    ...
    def _run_row(row: sqlite3.Row, with_spec: bool = False) -> Dict[str, Any]:
        result: Dict[str, Any] = {
            ...
            "run_id": row["run_id"] or row["workflow_id"],
            "task_id": row["task_id"],
            "lab_id": row["lab_id"],
