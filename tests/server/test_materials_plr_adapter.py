...
def test_plr_create_returns_server_uuid_and_all_substances(tmp_path) -> None:
    ...
    try:
        created = create_plr_materials(
            ...
            template_uuid_by_name={"beaker": "beaker-template"},
        )
        authoritative_uuid = created.result.data.root_material_uuid
        draft_uuid = next(iter(created.result.data.client_uuid_map))

        assert authoritative_uuid != draft_uuid
        assert beaker.unilabos_uuid == draft_uuid
