...
INSTANCE_TRANSITIONS: Dict[InstanceState, set] = {
    # 输出物料可能在入库/堆栈状态下被外部流程直接耗尽，允许 warehouse → consumed；
    # 仍保留 ledger 事件，不代表绕过预留消费。
    InstanceState.WAREHOUSE: {InstanceState.RESERVED, InstanceState.BENCH, InstanceState.STORED, InstanceState.CONSUMED, InstanceState.DISCARDED,
                              InstanceState.QUARANTINED},
    InstanceState.RESERVED: {InstanceState.WAREHOUSE, InstanceState.BENCH, InstanceState.QUARANTINED},
    InstanceState.BENCH: {InstanceState.IN_USE, InstanceState.WAREHOUSE, InstanceState.STORED, InstanceState.CONSUMED,
                          InstanceState.DISCARDED, InstanceState.QUARANTINED},
    InstanceState.IN_USE: {InstanceState.BENCH, InstanceState.STORED, InstanceState.CONSUMED, InstanceState.DISCARDED,
                           InstanceState.QUARANTINED},
    InstanceState.STORED: {InstanceState.WAREHOUSE, InstanceState.BENCH, InstanceState.IN_USE,
                           InstanceState.CONSUMED, InstanceState.DISCARDED, InstanceState.QUARANTINED},
