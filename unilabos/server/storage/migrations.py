...
from unilabos.server.scheduler.device_state import DEVICE_STATE_SCHEMA_VERSION
from unilabos.server.scheduler.inventory.store import SCHEMA_VERSION
...
from unilabos.server.workflow.schema import WORKFLOW_SCHEMA_VERSION
...
def build_store_migration_manifest(
    ...
    """为同一组解析路径建立三库清单，不打开任何数据库。

    ``paths`` 必须是组合根唯一解析出的路径对象；返回值固定覆盖工作流、
    库存和设备状态三个旧存储，供启动前校验其迁移所有者。
    """
    ...
    return {
        "workflow": StoreMigrationManifest(
            ...
            migration_owner="unilabos.server.workflow",
        ...
        "inventory": StoreMigrationManifest(
            ...
            migration_owner="unilabos.server.scheduler.inventory",
        ...
        "device_state": StoreMigrationManifest(
            ...
            migration_owner="unilabos.server.scheduler.device_state",
