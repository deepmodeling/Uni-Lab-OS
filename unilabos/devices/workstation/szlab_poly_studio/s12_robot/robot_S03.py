...
class SzlabRobotS03Mixin:
    ...
    def _run_s03_pick(self, product_type: int, position: str = "1-1") -> dict[str, Any]:
        ...
        if int(product_type) in (2, 3):
            beaker_sensor = product_slot_sensor(1, position, used=False)
            pre_sensor_conditions[beaker_sensor] = False
            post_sensor_conditions[beaker_sensor] = False
        return self._submit_robot_task(
            task="pick",
            station="S03",
            task_number=6,
            ...
            reset_variables={"S03取放料产品": 0, "S03取放料编号": 0, "任务号": 0},
            pre_sensor_conditions=pre_sensor_conditions,
            post_sensor_conditions=post_sensor_conditions,
            verify_reset=True,
            product_type=int(product_type),
            position=str(position),
